...
forecastData$Weekday <- as.numeric(forecastData$Weekday)
forecastData$hour <- as.numeric(forecastData$hour)
forecastData$month <- as.numeric(forecastData$month)
forecastData$Temperature <- as.numeric(forecastData$Temperature)
...
forecastData$kWh <- round(predict(lm.fit,forecastData),digits=0)

forecastData$Account <- NULL
forecastData$month=NULL
forecastData$day=NULL
forecastData$year=NULL
forecastData$Weekday=NULL
forecastData$Peakhour=NULL
forecastData$DayofWeek=NULL


#exporting the file
write.csv(forecastData, "forecast power usage old data.csv",row.names = FALSE)                            
